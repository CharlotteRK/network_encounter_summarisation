function[] = find_events(file, ap_file, out_file)
    ...
    begin_time = raw.start(1);
    raw.start = raw.start - begin_time;
    raw.fin = raw.fin - begin_time;
...
function convert_ids()
    global encounters
    sz = size(unique([encounters.id1;encounters.id2]));
    fprintf(1,'%d mobile devices(s) found...\n', sz(1)); 
    encounters = sortrows(encounters);
    ids1_map = table(unique([encounters.id1;encounters.id2]), (0:sz(1)-1)', 'VariableNames',{'id1', 'new_id'});
    ids2_map = table(unique([encounters.id1;encounters.id2]), (0:sz(1)-1)', 'VariableNames',{'id2', 'new_id'});
    joined = sortrows(outerjoin(encounters,ids1_map,'Key','id1'));
    fromMap = ~isnan(joined.time);
    encounters.id1 = joined.new_id(fromMap);
    joined = sortrows(outerjoin(encounters,ids2_map,'Key','id2'));
    fromMap = ~isnan(joined.time);
    encounters.id2 = joined.new_id(fromMap);
end
